...
def roll_dice(num_rolls, dice=six_sided):
    ...
    while num_rolls > 0:
        temp = dice()
        num_rolls , sum = num_rolls-1, sum+temp
        if(temp == 1):
            exist_1 = 1
    if(exist_1 == 1):
        return 1
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    # BEGIN PROBLEM 7
    last_a , running_ = last_score, running_high
    def say(score0, score1):
        if(who == 0):
            if(score0 - last_a > running_):
                print(score0 - last_a, 'point(s)!', "That's a record gain for Player 0!")
                return announce_highest(who , score0, score0-last_a)
            else:
                return announce_highest(who, score0, running_high)
        else:
            if(score1 - last_a > running_):
                print(score1 - last_a, 'point(s)!', "That's a record gain for Player 1!")
                return announce_highest(who, score1, score1-last_a)
            else:
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    max, i, av, maxav = 0 , 1, 0, 0
    while(i <= 10):
        av = average_score(i , dice)
        if(av > maxav):
            max , maxav = i, av
        i += 1
    return max
